...
// GetOffset returns HeadMetaData struct from container
func (hm *HeadMeta) GetOffset(index int) *HeadMetaData {
	if v := hm.Container().GetOffset(index); v != nil {
...
func (hm *HeadMeta) toString() string {
	items := []string{}
	//hm.Container().Sort()
	for _, item := range hm.Container().Get() {
		stringified, _ := hm.itemToString(item.(*HeadMetaData))
		// log.Notice(err)
