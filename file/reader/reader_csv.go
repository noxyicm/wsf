...
// A CSV reader reads records from a CSV-encoded file.
//
...
func NewCSVReader(r io.Reader) *CSV {
	return &CSV{
		Comma:           ',',
		StringDelimiter: '"',
		r:               bufio.NewReader(r),
...
func (r *CSV) Read() (record []string, err error) {
	if r.ReuseRecord {
...
// reported.
func (r *CSV) ReadAll() (records [][]string, err error) {
	for {
		record, err := r.readRecord(nil)
		if err == io.EOF {
			return records, nil
...
func (r *CSV) readLine() ([]byte, error) {
...
func (r *CSV) readRecord(dst []string) ([]string, error) {
...
parseField:
	for {
		...
		stringDelimiterByte := make([]byte, 1)
		utf8.EncodeRune(stringDelimiterByte, r.StringDelimiter)
		if len(line) == 0 || line[0] != stringDelimiterByte[0] {
			...
			if !r.LazyQuotes {
				if j := bytes.IndexByte(field, stringDelimiterByte[0]); j >= 0 {
			...
			line = line[quoteLen:]
			for {
				i := bytes.IndexByte(line, stringDelimiterByte[0])
				if i >= 0 {
					// Hit next quote.
					r.recordBuffer = append(r.recordBuffer, line[:i]...)
					line = line[i+quoteLen:]
					switch rn := nextRune(line); {
					case rn == r.StringDelimiter:
						// `""` sequence (append quote).
						r.recordBuffer = append(r.recordBuffer, stringDelimiterByte[0])
						line = line[quoteLen:]
					case rn == r.Comma:
